...
grep -q 'UNITY_INSTALL_ROOT=.*UNITY_APP/..' "$PACKAGE_SCRIPT"
grep -q 'UNITY_INSTALL_ROOT/PlaybackEngines/WindowsStandaloneSupport' "$PACKAGE_SCRIPT"
...
grep -q 'install-distribution-notices.sh' "$PACKAGE_SCRIPT"
grep -q 'SOURCE_COMMIT=.*HITTHEKIT_SOURCE_COMMIT' "$PACKAGE_SCRIPT"
grep -q 'playtest source must be committed and clean' "$PACKAGE_SCRIPT"
grep -q 'License, notices, and exact source information are in Legal/' "$PACKAGE_SCRIPT"
